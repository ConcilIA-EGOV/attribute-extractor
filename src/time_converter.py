...
def convert_time_to_numeric(csv_origin_path, res_dir_path):    
    ...
    if not csv_reader.fieldnames:
        print("Não foi possível ler o cabeçalho do arquivo")
        return

    int_extravio = int(INTERVALO_EXTRAVIO in csv_reader.fieldnames)
    int_atraso = int(INTERVALO_ATRASO in csv_reader.fieldnames)
    if not (int_extravio or int_atraso):
        print("Os campos de intervalo de extravio e de atraso não foram encontrados")
        return
    ...
    for row in csv_reader:
        try:
            ...
            if int_extravio:
                extravio = row[INTERVALO_EXTRAVIO]
            if int_atraso:
